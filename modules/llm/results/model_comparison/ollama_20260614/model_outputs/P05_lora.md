...
### Strengths
The paper presents DrQ-v2, a model-free reinforcement learning (RL) algorithm for visual continuous control. It builds on DrQ and introduces several improvements that yield state-of-the-art results on the DeepMind Control Suite. Notably, DrQ-v2 is able to solve complex humanoid locomotion tasks directly from pixel observations, previously unattained by model-free RL. The method is conceptually simple, easy to implement, and provides a significantly better computational footprint compared to prior work.

### Weaknesses
The paper does not explicitly discuss limitations. The supplied evaluation is scoped to DMC, so broader generalization is not established. The methodology section could benefit from more detail on the specific improvements made to DrQ-v2.

### Missing Evidence
A limitations discussion and clearly delimit claims to the evaluated benchmark are missing.

### Suggested Improvements
Add a limitations discussion and explicitly state the scope of claims to the evaluated benchmark. Include more detail on the specific improvements made to DrQ-v2 in the methodology section.

### Evidence Basis
[2107.09645v1_fd56aa5c6a]
