...
def cancel(request_id: str) -> bool:
    """Cancel one active request and signal its subprocess to stop.

    Only sends SIGTERM — does NOT wait for the process to exit.  The service
    thread that owns the subprocess (``_run_cli``) is responsible for reaping
    it; calling ``wait()`` here from a second thread races with the owner's
    ``communicate()`` / ``wait()`` calls and can hang indefinitely.
    """
    with _lock:
        entry = _active.get(request_id)
        if entry is None:
