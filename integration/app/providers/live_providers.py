...
def _run_cli(command, cwd, label, required_outputs, session=None):
    ...
    try:
        ...
        # The cancel endpoint may have sent SIGTERM while communicate() was in
        # progress, causing the process to exit within the 0.1 s window so
        # TimeoutExpired was never raised.  Check the flag here too.
        if is_cancelled():
            if session:
                session.log_subprocess_end(
                    command=command,
